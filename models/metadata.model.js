...
const validateMetadata = async function (next) {
    ...
    const checkMe = function (curFieldDesc, curValue) {
        ...
        if (curFieldDesc.fieldType.substring(0, 7) == 'SELECT_') {
            ...
            if (!theSet.has(curValue)) {
                return next(new Error('this field is not in the list of allowed values'));
            }
        }
    }
    ...
    if (fieldDesc.fieldType == 'TREE_FIELD_REPEATABLE' || fieldDesc.fieldType == 'TREE_FIELD') { //mxolod iseti ibieqtebi romelshic depended velebia
        ...
        for (const num of fieldDesc.childNums) {
            ...
            if (curFieldDesc.hasOwnProperty("dependsOn")) {
                ...
                const filteredByParent = globalEnums[num].filter(x => x.parent == parentValue)

                const curIndex = filteredByParent.findIndex(x => x.name == curValue);
                if (!curIndex) {
                    next(new Error('this field is not in the list of allowed values, from parent object '));
                }
            } else {
